...
/// Formulario de edicao de pessoa (ADR-011 / MVVM 1:1).
///
...
class PersonEditScreen extends ConsumerStatefulWidget {
  const PersonEditScreen({super.key, this.personId, this.person});
...
class _PersonEditScreenState extends ConsumerState<PersonEditScreen> {
  ...
  void initState() {
    ...
    _photoUrl = TextEditingController(text: person?.photoUrl ?? '');
    _city = TextEditingController(text: person?.city ?? '');
    _role = person?.role ?? 'athlete';
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final vm = ref.read(personEditViewModelProvider(widget.personId!));
  ...
  void _initFromPerson(dynamic person) {
    if (_initialized || person == null) return;
    _initialized = true;
    _name.text = person.name ?? '';
    _cpf.text = person.cpf ?? '';
    _photoUrl.text = person.photoUrl ?? '';
  ...
  Future<void> _save() async {
    ...
    if (ok) {
      ...
      setState(() {
        _errorMessage =
            vm.errorMessage ?? 'Nao foi possivel atualizar a pessoa.';
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final vm = ref.watch(personEditViewModelProvider(personId));

    _initFromPerson(vm.person);

    return PopScope(
      canPop: !_hasChanges || vm.isSubmitting || _saved,
