...
final approvalsBranch = StatefulShellBranch(routes: [
  GoRoute(path: '/approvals', name: 'approvals', builder: (c, s) => const ApprovalsScreen()),
]);

final institutionBranch = StatefulShellBranch(routes: [
  GoRoute(
    path: '/institutions',
    name: 'institutions',
    builder: (c, s) => const InstitutionListScreen(),
    routes: [
      GoRoute(path: 'new', name: 'institutionNew', builder: (c, s) => const InstitutionCreateScreen()),
      GoRoute(
        path: ':id',
        name: 'institutionDetail',
        builder: (c, s) {
          final inst = s.extra is Institution ? s.extra as Institution : null;
          return InstitutionDetailScreen(id: s.pathParameters['id']!, institution: inst);
        },
      ),
      GoRoute(
        path: ':id/edit',
        ...
        builder: (c, s) {
          final inst = s.extra is Institution ? s.extra as Institution : null;
          return InstitutionEditScreen(id: s.pathParameters['id']!, institution: inst);
        },
      ),
    ],
  ),
...
final usersBranch = StatefulShellBranch(routes: [
  GoRoute(
    path: '/users',
    name: 'users',
    builder: (c, s) => const UsersScreen(),
    routes: [
      GoRoute(path: 'new', name: 'userNew', builder: (c, s) => const UserFormScreen()),
    ],
  ),
]);
